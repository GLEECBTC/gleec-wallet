...
/// so that funds are never moved on behalf of a wallet the user has left.
...
Future<KdfUser?> _freshKdfCurrentUser(KomodoDefiSdk sdk) async {
  // Sandwich the daemon read between two auth-cache reads: if the session
  // changed while the RPC was in flight, the two cache reads disagree and the
  // answer is not authoritative for either wallet.
...
/// The compound ID of KDF's live wallet, or `null` when signed out.
///
/// Throws [KdfWalletAuthorityUnavailable] when authority cannot be established.
