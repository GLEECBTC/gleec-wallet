...
This is the KDF team hand-over for the future hardened Gleec Wallet TRON
GasFree releases:
...
It is a normative implementation contract for those future KDF changes, not an
audit report. The words **MUST**, **MUST NOT**, **SHOULD**, and **MAY** have
their usual RFC-style meanings.

> **Current V0 release contract.** V0 deliberately ships before either KDF
> enhancement profile in this document. It uses the manifest-pinned GasFree
> KDF commit and its legacy `gasless::account_status` response, with no GUI
> cryptography, Dart custody derivation, provider REST access, or new KDF source
> changes. Its executable policy is specified in
> `TRON_GASFREE_RECEIVE_CONTROL.md`, and its artifact/canary gates are specified
> in `TRON_GASFREE_V0_RUNBOOK.md`. A fresh, strictly validated legacy status may
> enable only the canonical core-wallet Receive surface behind the separate
> default-on `TRON_GASLESS_LEGACY_STATUS_RECEIVE_ENABLED` kill switch. It never
> authorizes Send, supplies final fees or Max, or enables integrations. The V1
> and V2 requirements below remain deferred hardening work.
...
The project has deliberately pivoted to a staged rollout. The current app
delivery is the constrained V0 described above. The KDF team's next delivery is
the **V1 status-attested opening**, not the full V2 contract. That later release
may proceed after the complete V1 delta from `bfd7f7ee…`, required tests, and
immutable platform artifacts pass the acceptance gate in section 20.1.
...
| --- | --- | --- | --- |
| V0 legacy-status opening | Existing `gasless::account_status` plus strict app/SDK policy; no KDF source delta | Canonical core-wallet Receive and preview-authoritative guarded Send; Standard TRON remains available | Current constrained implementation; rollout switches default on while artifact, design, and canary release gates remain explicit |
| V1 status-attested opening | Exact provider attestation, explicit availability, hard identity mismatches, balance-preserving non-usable states, timestamp compatibility, focused tests, and immutable artifacts | Core-wallet GasFree QR/copy for one canonical address; truthful pending/paused/recovery states; Standard TRON remains available | Deferred KDF hardening target |
...
- GasFree rollout switches default to enabled and remain explicit kill
  switches. Production must still provide a valid pinned provider
  configuration. When a network/provider-bound remote control URL is supplied,
  its fresh document remains authoritative and may disable new receives at any
  time;
...
- V0 may convert a fresh legacy `provider_available: true` response into the
  distinct `GaslessReceiveEvidence.legacyKdfAccountStatus` mode only after all
  canonical-asset, wallet, remote-control, epoch, pending-transfer, field,
  arithmetic, frozen-balance, and positive-fee checks pass. This evidence is
  core-wallet Receive-only and never becomes V1 evidence or Send authority;
- legacy `provider_available: false` remains balance/recovery compatibility
  data: it preserves the custody address and on-chain total but disables
  QR/copy and GasFree Send;
...
- V0 production core receive uses the default-on
  `TRON_GASLESS_ENABLED=true`,
  `TRON_GASLESS_RECEIVE_ENABLED=true`, and
  `TRON_GASLESS_LEGACY_STATUS_RECEIVE_ENABLED=true`, plus valid pinned
  provider/base configuration. An optional configured control URL additionally
  requires a fresh matching remote-control document; and
- `TRON_GASLESS_STATUS_ATTESTED_RECEIVE_ENABLED` also defaults on for future
  KDF responses. V0 legacy evidence takes deterministic precedence until KDF
  returns the explicit V1 contract; and
- all switches default to `true`. Disabling them hides new receive
