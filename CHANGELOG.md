...
- **SDK Release Line and Wallet-Identity RC** ([SDK#360](https://github.com/GLEECBTC/komodo-defi-sdk-flutter/pull/360), [SDK#367](https://github.com/GLEECBTC/komodo-defi-sdk-flutter/pull/367), [SDK#374](https://github.com/GLEECBTC/komodo-defi-sdk-flutter/pull/374); wallet #3528) - The submodule pins SDK [`f775840a`](https://github.com/GLEECBTC/komodo-defi-sdk-flutter/commit/f775840a3), the squash of [SDK#385](https://github.com/GLEECBTC/komodo-defi-sdk-flutter/pull/385) on SDK `dev`, which reaches `main` with [SDK#382](https://github.com/GLEECBTC/komodo-defi-sdk-flutter/pull/382), with SDK package version `0.8.0` and local-auth `0.6.0`. Metadata writes require the verified identity captured when work starts, and the app's callers are migrated. History keeps one wallet context across cached and live results, preserves streams during temporary identity failures, and rechecks the session before recording activation, including on WebAssembly. These packages are consumed through the Git submodule; the trading engine artefact was unchanged at `f3efd2c` for this release. The `add/routed-swap` branch has since repinned it to `feat/lifi-integration` `4872ef2`, which carries the routed-swap RPCs; that branch forks from `main` at `750be938` and does not include the five later `main` merges.
