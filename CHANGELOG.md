...
- **One Swap Surface** ([@CharlVS], [#3507](https://github.com/GLEECBTC/gleec-wallet/pull/3507)) - The Swap menu entry now opens a single surface with **Swap**, **Activity** and **Advanced** segments. Swap quotes across both liquidity sources - the atomic-swap orderbook and routed (aggregator-executed) swaps through the SDK's new `routed_swap` layer - and Advanced is the full trading interface, unchanged and one tap away. Activity merges routed and atomic swap history into one list.
